...
/**
	...
	protected function send($data,$command = false)
	{
		...
		$sendcommand = '';
		$unpack = unpack("C*",$sendData);
		foreach ($unpack as $value) 
		{
			$sendcommand.=sprintf("%02s",sprintf("%x",$value));
		}
		
		$this->setLogs("send command:$sendcommand");
		
		//显示命令
		if(SHOWCOMMAND===true)
		{
			system_out("command:".$sendcommand);
		}
	...
	protected function read($count = 128,$command = false)
	{
		...
		if ($this->timeOut)
		{
			...
			while ($while)
			{
				...
				if (false === $content)
				{
					if(11 == socket_last_error($this->socket))
					{
						usleep(1000*200);
					}
					else 
					{
						$this->setError = "Read failed! reason: " . socket_strerror(socket_last_error($this->socket));
						return false ;	
					}
		...
		foreach ($unpack as $value) 
		{
			$readdata.=sprintf("%02s",sprintf("%x",$value));
		}
		
		$this->setLogs("read result:$readdata");
		
		if(SHOWCOMMAND===true)
		{	
			system_out("readdata:".$readdata);
		}
