...
class DbMssql extends Db{
	protected $selectSql  =     'SELECT%DISTINCT% %LIMIT% %FIELDS% FROM %TABLE%%JOIN%%WHERE%%GROUP%%HAVING%%ORDER%';
    /**
     +----------------------------------------------------------
     * 架构函数 读取数据库配置信息
     +----------------------------------------------------------
    ...
    public function query($str) {
        ...
        //编码转换
        $str = auto_charset($str,C('DEFAULT_CHARSET'),C('DB_CHARSET'));
    ...
    private function getAll() {
        ...
        if($this->numRows >0) {
            while($row = mssql_fetch_assoc($this->queryID))
                $result[]   =   auto_charset($row,C('DB_CHARSET'),C('DEFAULT_CHARSET'));
        }
        return $result;
    }
    ...
    public function error() {
        $this->error = auto_charset(mssql_get_last_message(),C('DB_CHARSET'),C('DEFAULT_CHARSET'));
